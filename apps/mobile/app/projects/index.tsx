...
export default function ProjectsScreen() {
  ...
  return <ScrollView refreshControl={<RefreshControl colors={[colors.primary]} onRefresh={() => void loadProjects(true)} refreshing={loading} />} contentContainerStyle={[styles.container, { paddingBottom: insets.bottom + bottomNavigationHeight + spacing.xl }]}>
    <View style={styles.intro}><Text style={styles.muted}>Choose a workspace to review threads or start the next task.</Text></View>
    <View style={styles.headerRow}><SectionHeader count={projects.length} title="Workspaces" /><Link href="/projects/manage" asChild><Pressable accessibilityRole="button" testID="project-create-new"><Text style={styles.link}>New project</Text></Pressable></Link></View>
...
function ProjectCard({ project }: { project: ProjectSummary }) {
  const { primaryFolder, repositoryFolder } = workspaceFolderState(project);

  return <Link href={`/projects/${encodeURIComponent(project.project_id)}`} asChild><Pressable accessibilityLabel={`${project.name}, ${project.project_id}`} accessibilityRole="button" style={({ pressed }) => [styles.card, pressed && styles.pressed]} testID={`project-card-${project.project_id}`}><View style={styles.cardTop}><View style={styles.copy}><Text numberOfLines={1} style={styles.title}>{project.name}</Text><Text style={styles.projectId}>{project.project_id}</Text><Text style={styles.status}>{project.archived ? 'Archived workspace' : 'Active workspace'}</Text></View><StatusPill status={project.archived ? 'offline' : project.running_count > 0 ? 'running' : 'idle'} /></View><View style={styles.metadata}><MetadataRow label="Workspace" value={primaryFolder || 'No primary folder'} /><MetadataRow label="Repository" value={repositoryFolder || 'Workspace-only'} /><MetadataRow label="Attention" value={`${project.failed_count} failed`} /><MetadataRow label="Active" value={`${project.running_count + project.queued_count} tasks`} /><MetadataRow label="Completed" value={`${project.completed_count}`} /></View></Pressable></Link>;
}

const styles = StyleSheet.create({ card: { backgroundColor: colors.surface, borderRadius: 18, gap: spacing.md, padding: spacing.md }, cardTop: { alignItems: 'flex-start', flexDirection: 'row', gap: spacing.sm, justifyContent: 'space-between' }, container: { gap: spacing.lg, padding: spacing.lg }, copy: { flex: 1, gap: spacing.xs }, error: { color: colors.danger, fontSize: 14 }, headerRow: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }, link: { color: colors.primary, fontSize: 14, fontWeight: '800' }, metadata: { borderTopColor: colors.border, borderTopWidth: 1, gap: spacing.xs, paddingTop: spacing.sm }, muted: { color: colors.muted, fontSize: 14, lineHeight: 20 }, pressed: { opacity: 0.75 }, projectId: { color: colors.primary, fontSize: 12, fontWeight: '700' }, status: { color: colors.muted, fontSize: 12 }, title: { color: colors.text, fontSize: 17, fontWeight: '800' }, intro: { gap: spacing.xs } });
