...
> **Status:** Complete.
> **Progress:** 37 / 37 implementation tasks complete.
...
- [x] **P7.5** Perform physical-device validation: native project list, workspace creation, repository clone, task execution, retry/continuation, WebSocket reconnect, offline queue, and recovery-plan confirmation.
  **Completed device evidence:** loopback-only, marker-protected sandboxes on the authorized Android device verified native-project discovery/selection; task submission, detail, and fixture completion; managed workspace creation; clone-backed project creation from a committed local dumb-HTTP Git remote; retry/Continue action creating a linked completed attempt visible in the immutable two-attempt timeline; and explicit recovery-plan Restore confirmation with API postflight for registration, reclassification, and append-only audit data.
  **Offline queue procedure/result:** built and sideloaded the release APK, prepared a fresh marker-protected fixture on an unused loopback port, mapped only that port with `adb reverse`, and used `.maestro/p7-device-task.yaml` to cache the valid native project. After stopping the owned listener and removing the owned mapping, `.maestro/p7-offline-queue.yaml` visibly rendered the local queue's **Retry now** and **Discard** controls, then repeated those rendered assertions after an app stop/launch boundary. Physical taps verified Retry advanced the retained queue to a second local attempt and Discard removed it. After restoring only the fixture listener/mapping and restarting the app, the live authenticated API query found exactly one task for the queued request and the rendered queue controls disappeared after reconciliation.
  **WebSocket procedure/result:** with the same disposable fixture and mapped loopback port, `.maestro/p7-websocket-connection.yaml` saved only fixture runtime values and visibly established `connected`; `python scripts/device_sandbox.py disconnect --root <fixture-root> --port <fixture-port>` closed one authenticated fixture socket; `.maestro/p7-websocket-state.yaml` then visibly asserted `connected` and reconnect count `1`. The device Activity screen rendered the current queued/blocked fixture work-thread state after the replacement socket, matching the live authenticated `/work-threads` postflight.
  **Commands/verification:** `pnpm exec vitest run src/features/tasks/offline-queue.test.ts src/state/data-store.test.ts`; `pnpm run typecheck`; `python -m pytest -q scripts/test_device_sandbox.py`; `python scripts/verify.py --android --sideload`; fixture-backed Maestro flows above; live authenticated `/tasks` and `/work-threads` assertions. All passed on the release APK.
  **Cleanup:** stopped owned fixture listeners, removed only the owned reverse mapping, cleared the installed app's data, and destroyed the marker-protected fixture; verified no fixture listener, mapping, or fixture directory remained.
...
| 2026-07-21 | P7.5 offline queue and WebSocket reconnect | Fresh marker-protected loopback fixture on an unused port; release APK; `p7-device-task.yaml`, `p7-offline-queue.yaml`, `p7-websocket-connection.yaml`, and `p7-websocket-state.yaml`; authenticated `/tasks` and `/work-threads` postflight | passed | Offline flow rendered Retry/Discard and preserved them across app restart; Retry advanced the local queue, Discard removed it, restored connectivity reconciled exactly one server task and cleared the rendered queue. Sandbox disconnect closed one socket; device rendered connected with reconnect count 1 and current work-thread state after the replacement snapshot. Owned listener/mapping, app state, and marked fixture were removed. |
